...
from helpers import render, get_device_user, _check_rate_limit, _check_answer, _update_review_schedule, _grade_from_percentage, _award_xp, XP_REWARDS
from log import log
...
async def quiz_submit(data: QuizSubmitRequest, request: Request):
    ...
    for q in questions:
        ...
        await execute_async(
            ...
            "consecutive_correct = CASE WHEN last_correct = 1 AND %s = 1 THEN consecutive_correct + 1 "
    ...
    for q in questions:
        qid = q["id"]
        qid_str = str(qid)
        if qid_str in answers and _check_answer(q, answers[qid_str]):
            if q.get("difficulty", 1) >= 3:
                xp_earned += XP_REWARDS["quiz_correct_hard"]
            else:
                xp_earned += XP_REWARDS["quiz_correct"]
    if score_pct == 100 and correct_count > 0:
        xp_earned += XP_REWARDS["quiz_perfect"]
    if xp_earned > 0:
        _award_xp(user_id, xp_earned, "quiz")
