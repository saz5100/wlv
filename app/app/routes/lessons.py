...
from helpers import render, get_icon, get_device_user, _get_grade_summary
...
async def home(request: Request):
    ...
    return render("home.html",
        ...
        grade_summary=_get_grade_summary(user["id"]) if user else None
    )
...
async def lesson_page(lesson_id: int, request: Request):
    ...
    # Fetch related wiki concepts for this lesson's topic
    wiki_terms = []
    try:
        import psycopg2
        ...
        c = conn.cursor()
        c.execute("""
            SELECT ms.id, ms.key_terms, ms.model_answer
        ...
        for row in c.fetchall():
            terms = row[1] if row[1] else ""
            term_list = [t.strip() for t in terms.split(",") if t.strip()]
            if term_list:
                wiki_terms.append({
                    "id": row[0],
                    "terms": term_list[:6],  # max 6 terms per entry
                    "preview": row[2][:120] + "..." if row[2] and len(row[2]) > 120 else (row[2] or "")
                })
        conn.close()
    except Exception:
        pass  # Wiki DB unavailable — silently skip

    return render("lesson.html",
            active="lesson",
            lesson=lesson,
            content_rendered=render_mermaid_in_html(lesson["content"]),
...
async def progress_page(request: Request):
    ...
    weakest = await query_async("""
        ...
        JOIN topics t ON t.code = l.topic_code
        WHERE qm.user_id = %s
        GROUP BY l.id, l.title, l.topic_code, t.title
        HAVING SUM(qm.total_attempts) > 0
        ORDER BY CAST(SUM(qm.total_correct) AS REAL) / SUM(qm.total_attempts) ASC
        LIMIT 5
    """, (user_id,))
    for w in weakest:
...
async def search_page(q: str = Query("", max_length=100)):
    ...
    if query_text:
        like = f"%{query_text}%"
        rows = await query_async("""
            SELECT id, topic_code, title, content
            FROM lessons WHERE title ILIKE %s OR content ILIKE %s
            ORDER BY sort_order LIMIT 30
        """, (like, like))
        for r in rows:
            rd = dict(r)
            raw = rd.get("content", "") or ""
            # Truncate safely in Python (substr in SQL_ASCII counts bytes, can split emojis)
            if len(raw) > 200:
                raw = raw[:200]
