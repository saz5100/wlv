---
type: concept
title: Structural Features for Tension (AQA 2026)
created: 2026-06-11
updated: 2026-06-11
tags: [aqa, structure, tension, analysis, q3]
related: ["juxtaposition", "repetition-as-structural-device", "narrative-perspective-shifts"]
sources: ["AQA-8700-Sample-MS-Paper-1.md"]
---
# Structural Features for Tension (AQA 2026)

The AQA 8700/1 Sample Mark Scheme (June 2026) provides a list of structural features that writers use to create tension. These features are relevant to Q3 (structural analysis) and are included in the AO2 content list.

## List of Structural Features

- **Exposition:** Introduction of setting, characters, and background information
- **Foreshadowing:** Hints or clues about future events
- **Pivotal point:** A key moment where the narrative changes direction
- **Focus shifts:** Changes in narrative focus from one character, setting, or event to another
- **First-person perspective:** Narration from a single character's point of view
- **Motifs:** Recurring images, ideas, or symbols
- **Rising action:** Events that build tension towards a climax
- **Juxtaposition:** Placing contrasting elements side by side for effect
- **Anti-climax:** A disappointing or less dramatic resolution than expected

## How to Analyse These Features

When analysing structural features for tension, consider:

- **Purpose:** Why has the writer used this feature at this point?
- **Effect:** How does this feature create or build tension?
- **Placement:** Where in the text does this feature appear?
- **Relationship:** How does this feature relate to other structural features?

## Example from the Sample Mark Scheme

The indicative standard for Q3 demonstrates analysis of foreshadowing, pivotal point, and first-person perspective in the *Life of Pi* extract. The writer uses these features to create tension around the hyena's behaviour.

## Related Pages

- narrative-structure — Narrative structure
- [[juxtaposition]] — Placing contrasting elements side by side
- [[repetition-as-structural-device]] — Repetition as a structural device
- [[narrative-perspective-shifts]] — Changes in narrative focus

# ─── Enrichment ──────────────────────────────────────────
# Golden Words: tension, perspective, focus, shift, repetition, foreshadowing, climax, rising action, resolution, exposition
# Assessment Objectives: AO1, AO2
# Causal Links:
# - The structural shift from foreshadowing creates tension, which causes the reader to feel engaged.
# - By using juxtaposition, the writer establishes atmosphere, leading the reader to sympathise with the character.
# - The repetition at this point in the text creates suspense, which makes the reader understand the setting.


# ─── Enrichment ──────────────────────────────────────────
# Golden Words: tension, shift, repetition, perspective, foreshadowing, focus, climax, rising action, resolution, exposition
# Assessment Objectives: AO1, AO2
# Causal Links:
# - The structural shift from foreshadowing creates tension, which causes the reader to the reader to engage with the text.
# - By using juxtaposition, the writer establishes atmosphere, leading the reader to the reader to sympathise with the character.
# - The repetition at this point in the text creates suspense, which makes the reader the reader to feel engaged.


# ─── Enrichment ──────────────────────────────────────────
# Golden Words: tension, shift, repetition, foreshadowing, perspective, focus, climax, rising action, resolution, exposition
# Assessment Objectives: AO1, AO2
# Causal Links:
# - The structural shift from foreshadowing creates tension, which causes the reader to engage with the text.
# - By using juxtaposition, the writer establishes atmosphere, leading the reader to sympathise with the character.
# - The repetition at this point in the text creates suspense, which makes the reader feel engaged.
