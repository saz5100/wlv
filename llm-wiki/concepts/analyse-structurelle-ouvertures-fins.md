---
type: structural-analysis
title: Analyse Structurelle des Ouvertures et Fins
created: 2026-06-11
updated: 2026-06-11
tags: [structural-analysis, openings, endings, paper1-q3]
related: [bitesize-aqa-revision-guide, sequencement-chronologique, sequencement-non-lineaire, repetition-structurelle, juxtaposition-structurelle]
sources: ["bitesize-aqa-revision-guide.md"]
---
# Analyse Structurelle des Ouvertures et Fins

Présentation des techniques d'ouverture et de fin dans le guide BBC Bitesize pour Paper 1, Question 3 (analyse de la structure).

## Techniques d'ouverture

- Planter le décor
- Introduire un personnage
- Plonger dans l'action (in medias res)
- Retenir des informations pour créer du mystère

## Techniques de fin

- Résoudre le conflit
- Cliffhanger
- Structure cyclique (début et fin similaires)
- Résumé ou conclusion

Les structures cycliques attirent l'attention sur un thème ou un comportement.

# ─── Enrichment ──────────────────────────────────────────
# Golden Words: in medias res
# Assessment Objectives: AO1, AO2
# Causal Links:
# - The structural shift from in medias res creates tension, which causes the reader to feel engaged.
# - By using cliffhanger, the writer establishes atmosphere, leading the reader to sympathise with the character.


# ─── Enrichment ──────────────────────────────────────────
# Golden Words: in medias res, tension, shift
# Assessment Objectives: AO1, AO2
# Causal Links:
# - The structural shift from in medias res creates tension, which causes the reader to the reader to engage with the text.
# - By using cliffhanger, the writer establishes atmosphere, leading the reader to the reader to sympathise with the character.


# ─── Enrichment ──────────────────────────────────────────
# Golden Words: in medias res, tension, shift
# Assessment Objectives: AO1, AO2
# Causal Links:
# - The structural shift from in medias res creates tension, which causes the reader to engage with the text.
# - By using cliffhanger, the writer establishes atmosphere, leading the reader to sympathise with the character.
